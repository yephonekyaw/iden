...
class TestRegistration:
    async def test_confidential_client_secret_is_shown_once(self, client, admin_headers):
        created = (await client.post("/admin/clients", json=SERVICE, headers=admin_headers)).json()
        assert created["clientSecret"]

        fetched = (await client.get(
            f"/admin/clients/{created['id']}", headers=admin_headers
        )).json()
    ...
    async def test_public_client_gets_no_secret(self, client, admin_headers):
        ...
        created = (await client.post("/admin/clients", json=WEB_APP, headers=admin_headers)).json()
    ...
    async def test_secret_is_not_stored_in_the_clear(self, client, admin_headers, db):
        ...
        created = (await client.post("/admin/clients", json=SERVICE, headers=admin_headers)).json()
        stored = await db.scalar(select(Client).where(Client.client_id == "nightly-job"))

        assert stored.client_secret_hash != created["clientSecret"]

    async def test_authorization_code_grant_requires_a_redirect_uri(self, client, admin_headers):
    ...
    async def test_relative_redirect_uri_is_rejected(self, client, admin_headers):
        response = await client.post(
            "/admin/clients", json=WEB_APP | {"redirectUris": ["/callback"]}, headers=admin_headers
        )
        assert response.status_code == 422
...
class TestSecretRotation:
    async def test_rotation_invalidates_the_previous_secret(self, client, admin_headers, catalogue):
        created = (await client.post("/admin/clients", json=SERVICE, headers=admin_headers)).json()
        ...
        with_old = await client.post(
            "/oauth2/token",
            data={"grant_type": "client_credentials", "client_id": "nightly-job",
...
class TestClientScopes:
    async def test_grantable_and_granted_are_independent(self, client, admin_headers, catalogue):
        created = (await client.post("/admin/clients", json=SERVICE, headers=admin_headers)).json()
    ...
    async def test_granted_scopes_reach_a_client_credentials_token(
        ...
        token = (await client.post(
            "/oauth2/token",
            data={"grant_type": "client_credentials", "client_id": "nightly-job",
                  "client_secret": created["clientSecret"], "scope": "entity:profile:read"},
        )).json()
...
class TestProtection:
    async def test_bootstrap_client_cannot_be_deleted(self, client, admin_headers, db, dashboard):
        dashboard.is_system = True
        db.add(dashboard)
        await db.commit()

        response = await client.delete(f"/admin/clients/{dashboard.id}", headers=admin_headers)
